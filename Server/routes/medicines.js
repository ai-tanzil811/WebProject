...
router.get('/', async (req, res) => {
  try {
    ...
    if (search) {
      query += ' AND (generic_name LIKE ? OR brand_name LIKE ? OR description LIKE ?)';
  ...
  try {
    ...
    try {
      const [usageRows] = await connection.query(
        'SELECT COUNT(*) AS linked_order_items FROM OrderItems WHERE medicine_id = ?',
        [id]
      );

      if (usageRows[0].linked_order_items > 0) {
        return res.status(409).json({
          success: false,
          message: 'This medicine cannot be deleted because it is included in existing orders'
        });
      }
...
router.post('/search', async (req, res) => {
  try {
    ...
    try {
      const [medicines] = await connection.query(
        'SELECT medicine_id, generic_name, brand_name, strength, dosage_form, price FROM Medicines WHERE (generic_name LIKE ? OR brand_name LIKE ? OR description LIKE ?) LIMIT ?',
