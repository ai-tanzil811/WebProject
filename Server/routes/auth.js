...
if (emailUser && emailPass) {
  ...
  console.warn('⚠️ Email transporter not initialized: EMAIL_USER and EMAIL_PASSWORD are required');
}
...
function saveUploadedProfileFile(file, folder) {
  const uploadsDir = path.join(__dirname, '..', '..', 'FrontEnd', 'uploads', folder);
...
router.post('/register-user', upload.single('photo'), async (req, res) => {
  try {
    ...
    try {
      ...
      if (req.file) {
        photoPath = saveUploadedProfileFile(req.file, 'users');
...
router.post('/register-admin', upload.single('shopBanner'), async (req, res) => {
  try {
    ...
    try {
      ...
      if (req.file) {
        bannerPath = saveUploadedProfileFile(req.file, 'admins');
...
// Uses memory multer and writes files into FrontEnd/uploads so static serving can expose them.
router.patch('/profile', upload.single('photo'), async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      photoPath = saveUploadedProfileFile(req.file, role === 'Admins' ? 'admins' : 'users');
      photoType = req.file.mimetype;
    }
