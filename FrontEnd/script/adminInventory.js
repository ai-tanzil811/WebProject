document.addEventListener("DOMContentLoaded", () => {
  ...
  const paginationWrap = document.getElementById("inventoryPagination");
  const prevPageBtn = document.getElementById("inventoryPrevPage");
  const nextPageBtn = document.getElementById("inventoryNextPage");
  ...
  const getPaginationMeta = (totalItems) => {
  ...
  const renderPagination = (totalItems) => {
    if (!paginationWrap || !prevPageBtn || !nextPageBtn || !pageInfo) {
      return;
    }
    ...
    const { totalPages } = getPaginationMeta(totalItems);
  ...
  const renderRows = (items) => {
    ...
    const { startIndex, endIndex } = getPaginationMeta(items.length);
    ...
    setStatusMessage(`Showing ${pageItems.length} of ${items.length} medicine${items.length === 1 ? "" : "s"} in the current view.`, "muted");
  };
